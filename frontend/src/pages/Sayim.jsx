  ...
  const kaydet = async () => {
    ...
    try {
      const detaylar = urunler.map(u => {
        ...
        const fark     = sayilan - beklenen;
        return {
          urun_id: u.id, urun_ad: u.name, beklenen, sayilan,
    ...
    <div>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <select value={selMak} onChange={e => setSelMak(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: '1.5px solid var(--border)', borderRadius: 10, padding: '10px 12px', color: 'var(--text)', fontSize: 13, outline: 'none', WebkitAppearance: 'none' }}>
        ...
        <div style={{ overflowX: 'auto', maxHeight: 340, overflowY: 'auto', marginBottom: 14 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>{['Ürün', 'Beklenen', 'Sayılan', 'Fark'].map(h => (
                ...
                return (
                  <tr key={u.id}>
                    ...
                    <td style={{ padding: '7px 10px' }}>
                      <input type="number" min="0" value={sayilar[u.id] ?? ''} placeholder="0"
        ...
        <button onClick={kaydet} disabled={loading} style={{
          width: '100%', padding: 12, borderRadius: 10, border: 'none',
          background: 'var(--accent)', color: '#0d0f14', fontWeight: 700, fontSize: 13,
        }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr>{['Otomat', 'Ürün', 'Beklenen', 'Sayılan', 'Fark', 'Ciro', 'Tarih'].map(h => (
