...
export default function Stok({ showToast }) {
  ...
  const [form, setForm] = useState({
    urun_id: '', fiyat: '', miktar: '', birim: 'Adet', fatura_no: '', hedef: 'depot'
  });
  ...
  const calcHesap = () => {
    ...
    const qty = parseFloat(form.miktar) || 0;
    const fiyat = parseFloat(form.fiyat) || 0;
    if (!u || !qty || !fiyat) return setHesap(null);
    const eskiStok = parseInt(u.stock) || 0;
    const eskiMaliyet = parseFloat(u.cost) || 0;
    const toplam = qty * fiyat;
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
          <div>
            ...
            <select {...inp('urun_id')} style={{ ...inp('urun_id').style, WebkitAppearance: 'none', fontSize: 12 }}>
          ...
          <div style={{ background: 'rgba(0,229,160,0.05)', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 12, padding: 14, marginBottom: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              {[
                ...
                <div key={i} style={{ textAlign: 'center', padding: '8px', borderRight: i < 2 ? '1px solid var(--border)' : 'none' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
        ...
        <div style={{ overflowX: 'auto', maxHeight: 300, overflowY: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>{['Ürün', 'Miktar', 'Birim', 'Alış Fiyatı', 'Fatura', 'Tarih'].map(h => (
