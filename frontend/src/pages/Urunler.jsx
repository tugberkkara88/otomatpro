...
export default function Urunler({ showToast }) {
  ...
  const scanner = useBarcodeScanner({
    onDetected: (barcode) => {
      ...
      else showToast('📦 Barkod tarandı: ' + barcode, 'info');
    },
  });
  ...
  const handleCSV = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      try { const result = await api.urunler.bulk(rows); showToast(`✅ ${result.added} eklendi, ${result.updated} güncellendi`); load(); }
      catch(e) { showToast('❌ ' + e.message, 'err'); }
      e.target.value = '';
    };
  ...
  return (
    <div>
      <BarcodeModal scanner={scanner} title="Ürün Barkodu Tara" />
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, padding:16, marginBottom:14 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:8 }}>
          <div><label style={{ fontSize:11, fontWeight:700, color:'var(--muted)', display:'block', marginBottom:4 }}>Kategori</label>
            <select {...inp('cat')} style={{ ...inp('cat').style, WebkitAppearance:'none' }}>{cats.map(c => <option key={c}>{c}</option>)}</select></div>
          <div><label style={{ fontSize:11, fontWeight:700, color:'var(--muted)', display:'block', marginBottom:4 }}>Birim</label>
            <select {...inp('unit')} style={{ ...inp('unit').style, WebkitAppearance:'none' }}>{units.map(u => <option key={u}>{u}</option>)}</select></div>
          <div><label style={{ fontSize:11, fontWeight:700, color:'var(--muted)', display:'block', marginBottom:4 }}>ABC Sınıfı</label>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12 }}>
          {['f1','f2','f3'].map((f,i) => (
            <div key={f}><label style={{ fontSize:11, fontWeight:700, color:'var(--muted)', display:'block', marginBottom:4 }}>{['💵 Fiyat 1','💴 Fiyat 2','💶 Fiyat 3'][i]}</label>
              <input {...inp(f)} type="number" placeholder="0.00" step="0.01"
                style={{ ...inp(f).style, borderColor:['var(--accent)','var(--accent2)','var(--accent3)'][i] }} /></div>
          ))}
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={save} style={{ flex:1, padding:'11px 16px', borderRadius:10, border:'none', background:'var(--accent)', color:'#0d0f14', fontWeight:700, fontSize:13 }}>
            {editId ? '✅ Güncelle' : '💾 Kaydet'}
          </button>
          {editId && <button onClick={() => { setEditId(null); setForm({ barcode:'', name:'', cat:'📦 Diğer', unit:'Adet', abc:'B', f1:'', f2:'', f3:'' }); }}
            style={{ padding:'11px 16px', borderRadius:10, border:'1px solid var(--border)', background:'var(--surface2)', color:'var(--text)', fontWeight:700 }}>✕ İptal</button>}
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, padding:16 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
          <div style={{ fontFamily:'var(--font-head)', fontSize:14, fontWeight:700 }}>Ürün Listesi ({urunler.length})</div>
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            <ScanBtn onClick={scanner.open} style={{ padding:'6px 10px', fontSize:14 }} />
        ...
        {loading ? <div style={{ textAlign:'center', color:'var(--muted)', padding:30 }}>Yükleniyor...</div> : (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              <thead><tr>{['Ürün','Barkod','Birim','ABC','F1','F2','F3','Maliyet','Stok','İşlem'].map(h => (
                  ...
                  <tr key={u.id}>
                    ...
                    <td style={{ padding:'9px 10px' }}>
                      <span style={{ padding:'2px 7px', borderRadius:6, fontSize:10, fontWeight:700,
                        ...
                        color:u.abc==='A'?'var(--accent)':u.abc==='B'?'var(--accent2)':'var(--warn)' }}>{u.abc}</span>
                    </td>
                    ...
                    <td style={{ padding:'9px 10px', fontFamily:'var(--font-mono)', fontWeight:600,
                      color:u.stock<10?'var(--danger)':u.stock<50?'var(--warn)':'var(--text)' }}>{u.stock}</td>
                    <td style={{ padding:'9px 10px' }}>
